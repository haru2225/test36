#!/usr/bin/env python3
"""Clay CG workflow using test32's unchanged NequIP displacement denoiser.

prepare -> train -> generate / export / md -> analyze
No scalar energy, condition embedding, equilibrium claim or physical clock
calibration is introduced. See TEST36.md for input mapping and limitations.
"""
...
DM2_ROOT = Path(os.environ.get("DM2_ROOT", ROOT / "DM2")).expanduser().resolve()
...
FORMAT = "test36-clay-cg-denoiser-v1"
CAVEAT = (
...
def load_dataset(folder):
    ...
    if meta.get("format") != FORMAT or meta.get("length_unit") != "angstrom":
        raise ValueError("Expected test36 dataset with explicit angstrom units")
...
def checkpoint_model(path, device):
    ...
    if ck.get("format") != FORMAT:
        raise ValueError("Expected test36 checkpoint; SiO2 weights are not clay weights")
...
def generate(args):
    ...
    if args.resume:
        ...
        if state["settings"] != settings:
            raise ValueError("Generation resume settings differ")
    ...
    for step in range(completed, total):
        if STOP or time.monotonic() >= deadline:
            ...
            return 75
        data = graph(pos.cpu().numpy(), cell, meta["type_ids"], ck["large_cutoff"], device)
...
def export(args):
    ...
    atoms.wrap()
    lines = ["test36 CG start (species order in bundle JSON)", "", f"{len(atoms)} atoms",
...
def parser():
    ...
    p.add_argument("--cutoff", type=positive, default=5.0)
